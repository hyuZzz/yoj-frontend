  ...
  <div id="questionSubmitView">
    ...
    >
      <template #judgeInfo="{ record }">
        <a-space wrap>
          <template v-for="(info, index) of record.judgeInfo">
            <a-tag
              size="medium"
              :key="index"
              :color="
      ...
      <template #submitState="{ record }">
        ...
        <a-tag v-if="record.submitState === 0" color="cyan">等待中</a-tag>
        <a-tag v-if="record.submitState === 1" color="green">判题中</a-tag>
        <a-tag v-if="record.submitState === 2" color="blue">完成</a-tag>
        <a-tag v-if="record.submitState === 3" color="red">编译失败</a-tag>
        <a-tag v-if="record.submitState === 4" color="red">运行失败</a-tag>
      </template>
    </a-table>
  </div>
</template>
